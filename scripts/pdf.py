...
def _parse_toc (doc):
	...
	"""打开一个pdf文档对象，获得一个包含数据内容的表
	这里以一个高阶函数传递给with_pdf()
	"""
    toc = []
    try:
...
def save_image (lt_image, page_number, images_folder):
	...
	"""尝试从LTImage对象中保存图片文件，如果成功就返回文件名 """
    result = None
    if lt_image.stream:
...
def to_bytestring (s, enc='utf-8'):
	...
	"""将unicode字符串转换成比特流，使用标准编码，除非已经是比特字符串 """
    if s:
        if isinstance(s, str):
            return s
        else:
...
def update_page_text_hash (h, lt_obj, pct=0.2):
	...
	"""把所有x0且x1的坐标相差在20%以内的元素分成1组，实现从pdf文件中定向抽取内容 """
